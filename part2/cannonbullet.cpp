...
const QSize CannonBullet::ms_fixedSize(8, 8);

CannonBullet::CannonBullet(QPoint startPos, QPoint targetPoint, int damage, Enemy *target,
...
void CannonBullet::draw(QPainter *painter) const
{
...
void CannonBullet::hitTarget()
{
    ...
    if (m_game->enemyList1().indexOf(m_target) != -1)
        m_target->getDamage(m_damage);
    m_game->removedCannonBullet(this);
}

void CannonBullet::setCurrentPos(QPoint pos)
{
    m_currentPos = pos;
}

QPoint CannonBullet::currentPos() const
{
    return m_currentPos;
}
